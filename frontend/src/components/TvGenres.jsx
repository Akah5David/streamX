...
export default function TvGenres({ tvGenres, tvGenreRef }) {
  ...
  return (
    <section ref={tvGenreRef} className="relative inset-0 py-4">
      <SliderButton handleNext={handleNext} handlePrev={handlePrev} />
        ...
        >
          {tvGenres.map((movie) => (
            <motion.li
              key={`${movie.id}`}
              className="flex-none border-1 w-[70%] sm:w-[45%] md:w-[30%] lg:w-[20%] xl:w-[15%] aspect-1/1.5 rounded-lg object-cover snap-center snap-always scroll-m-6 bg-blue-500 hover:scale-105 overflow-hidden"
            >
              <Link
