...
export default function Others({ others, section, genre, classification }) {
  ...
  return (
    <>
    <h2 className = "font-bold text-4xl uppercase ">Related Videos</h2>
      <div className="grid grid-auto-flow: row grid-cols-5 gap-5 pl-4 pr-8">
        {others.map((o) => (
          <button
            onClick={() => handleCardClick(o.id)}
            key={o.id}
            className=" flex flex-col h-full gap-4 border-1 rounded-lg overflow-hidden cursor-pointer hover:opacity-80 transition-opacity"
          >
